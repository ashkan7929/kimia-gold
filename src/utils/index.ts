...
export default function ValidNationalCode(input: string): boolean {
   if (input.length < 10 || input.length > 10) {
        return false;
    }
    if (!/^\d{10}$/.test(input) || /^(\d)\1{9}$/.test(input)) {
        return false;
    }

    const code = Array.from(input, Number);
    const check = code[9];

    const sum = code.slice(0, 9).reduce((acc, num, i) => acc + num * (10 - i), 0);
    const remaining = sum % 11;
    const expected = remaining < 2 ? remaining : 11 - remaining;
