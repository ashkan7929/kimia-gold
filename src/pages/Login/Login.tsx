...
const Login = () => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (!phoneNumber || !/^09\d{9}$/.test(phoneNumber)) {
            setPhoneError('شماره موبایل به درستی وارد نشده است');
            return;
        }
        ...
        try {
            if (step == RegisterType.None) {
                ...
                const res = await axios.post(
                    'http://62.3.41.64:5016/api/auth/login-with-otp',
                    data,
                );
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full">
                    ...
                    {step == RegisterType.None && (
                        <>
                            ...
                            />
                            {phoneError && (
                                <p className="text-red-600 text-xs font-peyda mt-1 mr-3 font-light">
                                    {phoneError}
                                </p>
                            )}
                        </>
