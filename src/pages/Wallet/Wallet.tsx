      ...
      <main className="px-4 py-6 flex flex-col items-center gap-6">
                    ...
                    return (
                      <div
                        ...
                        className={`min-w-[300px] max-w-[300px] cursor-pointer transition-all duration-300 ${
                          isSelected
                            ? 'dark:bg-black bg-light-primary-darker scale-105 shadow-lg'
                            : 'dark:bg-dark-900 bg-light-primary-light dark:hover:bg-primary-darker/70 hover:bg-light-primary-darker'
                        } flex p-4 rounded-lg`}
                        ...
                        <main className="flex-grow flex gap-3 flex-col bg-[url('/images/Lines-pattern-starters.png')] bg-cover bg-center">
                          ...
                          <div className="grid grid-cols-2 gap-2 mt-3">
                            <button
                              ...
                              className="bg-accent-orange text-white hover:opacity-90 transition-colors text-[10px] font-peyda px-3 py-2 rounded-md flex items-center justify-center gap-1"
                            >
                              افزایش موجودی
                      ...
                      return (
                        <button
                          ...
                          className={`cursor-pointer px-4 py-2 rounded-md transition-colors ${
                            isSelected ? 'bg-accent-orange' : 'bg-transparent hover:bg-white/5'
                          }`}
                        >
                          <Typography
                            className={`!font-kalameh text-nowrap font-semibold ${
                              isSelected ? '!text-white' : 'dark:text-text-color text-light-text-color'
                            }`}
                            fontSize={11}
                          >
                            {tab.title}
                    ...
                    .filter(tr => {
                      ...
                      if (selectedTab.id === 3)
                        return (
                          ...
                          tr.transactionTypeName?.toLowerCase().includes('transfer')
                        );
                      return true;
                    })
                    .map((tr, idx) => (
                      <div key={idx} className="dark:bg-black bg-light-primary-light p-3 rounded-lg">
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <Typography className="!font-peyda text-custom-gray" fontSize={10}>
                          ...
                          <div className="text-left">
                            <Typography
                              className={`!font-peyda ${
                                tr.amount > 0 ? 'text-green-500' : 'text-red-500'
                              }`}
                              fontWeight="bold"
                              fontSize={14}
                            >
                        ...
                        {tr.processedAt && (
                          <div className="mt-2 pt-2 border-t border-custom-gray/20">
                            <Typography className="!font-peyda text-custom-gray" fontSize={9}>
                              {new Date(tr.processedAt).toLocaleDateString('fa-IR')}
                            </Typography>
                          </div>
                        )}
                      </div>
        ...
        >
          <div className="flex flex-col gap-3">
            <div className="flex gap-1 items-center">
              ...
              <Typography className="!font-peyda dark:text-text-color text-light-text-color" fontWeight={600} fontSize={13}>
                مبلغ افزایش موجودی
              </Typography>
            </div>
            ...
            <FormControl
              ...
              size="small"
              sx={{
                '.MuiOutlinedInput-notchedOutline': { borderColor: isDark ? '#3b82f6' : 'gray' },
                '.MuiSvgIcon-root, .MuiSelect-icon': { color: isDark ? 'white' : 'black' },
              ...
              <Select
                ...
                renderValue={() => (
                  <Stack direction="row" alignItems="center" spacing={1.25}>
                    <Box component="img" src={selected.icon} alt="" sx={{ width: 20, height: 20 }} />
                    <Box>
                      <Typography
                        sx={{
                ...
                MenuProps={{
                  PaperProps: {
                    sx: { bgcolor: isDark ? '#0b1c7a' : 'white', color: isDark ? 'white' : 'black' },
                  },
                }}
                sx={{ '.MuiSelect-select': { display: 'flex', alignItems: 'center', py: 1.25 } }}
              >
                {options.map(opt => (
                  <MenuItem key={opt.value} value={opt.value}>
                    <Stack direction="row" spacing={1.25} alignItems="center" sx={{ width: '100%' }}>
                      <Box component="img" src={opt.icon} alt="" sx={{ width: 20, height: 20 }} />
                      <Box>
                        <Typography
                          sx={{
                            color: isDark ? 'white' : 'black',
                        ...
                        <Typography sx={{ color: isDark ? 'rgba(255,255,255,0.65)' : 'rgba(0,0,0,0.6)', fontSize: 12 }}>
                          {opt.bankName}
                        </Typography>
                      </Box>
                    </Stack>
        ...
        >
          <div className="flex flex-col gap-3">
            ...
            <select className="!font-peyda w-full bg-transparent bg-white border dark:border-custom-border-light border-custom-gray text-xs rounded-lg px-4 py-3 dark:text-white text-light-text-color focus:outline-none focus:ring-2 focus:ring-accent-orange focus:border-transparent">
        ...
        >
          <div className="flex flex-col gap-3">
            <div className="flex gap-1 items-center">
              ...
              <Typography className="!font-peyda text-text-color dark:text-light-text-color" fontWeight={600} fontSize={13}>
                مبلغ انتقالی
              </Typography>
            </div>
