...
import {
    moreLight,
    moreDark,
    insuranceServicelight,
    insuranceServices,
    wealthManagementLight,
    wealthManagementDark,
    creditServicesLight,
    creditServicesDark,
    shoppingIconLight,
    shoppingIcon,
...
const themedSrc = (isDark: boolean, darkSrc: string, lightSrc: string) =>
...
const BalanceOrWelcome = ({
    ...
    if (showWelcome) {
        ...
        return (
            <div className="flex items-center gap-1 bg-white dark:bg-black rounded-xl p-3">
    ...
    return (
        <div className="dark:bg-black bg-light-primary-darker w-full font-peyda dark:text-text-color text-light-text-color flex items-center justify-between p-2 rounded-lg">
            <div className="flex items-center gap-2">
                <img
                    alt="Wallet logo"
                    src={themedSrc(isDark, walletLogo, lightwallet)}
                    width={30}
                    height={30}
                />
        ...
        <div className="dark:odd:bg-black odd:bg-white grid grid-cols-9 w-full">
            <div className="flex gap-1 items-center col-span-3 py-2">
                ...
                <div className="w-6.5 h-6.5 flex justify-center items-center rounded-full bg-gold-100">
                    <Link
                        to={`/product?id=${row.id}`}
                        aria-label={`${t('tableHeadView')} ${row.name}`}
                    >
...
const Home = () => {
    ...
    const userData = useMemo(() => {
        ...
        try {
            return JSON.parse(localStorage.getItem('user-data') || '{}') as {
                firstName?: string;
                lastName?: string;
            };
        } catch {
    ...
    // }, [user?.id]);
    useEffect(() => {
        const welcomeKey = `seen_welcome_${user?.id ?? 'anon'}`;
