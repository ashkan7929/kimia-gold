...
const Buy = () => {
  ...
  // const userId = "4c5d83f3-b90e-439a-88b6-1b2f6386d615";  
  ...
  const handleBuy = async () => {
    if (!weight || typeof weight !== 'number') {
      alert("لطفاً مقدار طلای موردنظرتان را انتخاب کنید");
      return;
    }
...
const token = localStorage.getItem('auth.token') || localStorage.getItem('token');
      const response = await axios.post('https://vemclub.com/api/api/Order', orderData, {
          headers: {
    Authorization: `Bearer ${token}`,
  },
      });  
      
      console.log('سفارش ثبت شد:', response.data);
      
      setWeight('');
      alert("سفارش با موفقیت ثبت شد!");
      
    } catch (err: any) {
      console.error('سفارش شکست خورد:', err);
                ...
                <div className="w-full rounded-lg bg-white dark:bg-black p-1 transition-all duration-700 ease-out">
                  ...
                  >
                    <Disclosure.Panel as="div" className="overflow-hidden">
                      <div className="flex flex-col gap-3 p-4">{/* محتوای اضافی اگر لازم */}</div>
                    </Disclosure.Panel>
                  </Transition>
                </div>
              )}
            ...
            <div className="rounded-lg bg-white dark:bg-black p-4">
              ...
              <div className="block" tabIndex={0}>
                <div className="mt-3 flex flex-col gap-3">
                  ...
                  {/* نمایش error */}
                  {error && (
                    <div className="text-red-500 text-xs mt-2 p-2 bg-red-100 dark:bg-red-900 rounded">
                      {error}
                    </div>
                  )}
                  ...
                  <div>
                    <button
                      ...
                      className="w-full p-3 rounded-lg dark:bg-accent-orange bg-primary-blue hover:bg-primary-blue text-sm text-white"  // className درست شد
                    >
                      {isLoading ? 'در حال ثبت سفارش...' : 'خرید طلا'}  
                    </button>
                  </div>
                </div>
              </div>
