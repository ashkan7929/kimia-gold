...
const OptionSelect = ({
  ...
  return (
    ...
    >
      ...
      {options.map((opt) => (
        <option key={opt.value} value={opt.value} className="bg-primary-dark light:bg-white light:border light:border-gray-100 light:outline-dashed light:outline-10">
          <Typography className="text-white font-peyda light:text-light-text-color" fontSize={10}>
            {opt.label}
          </Typography>
        </option>
      ))}
